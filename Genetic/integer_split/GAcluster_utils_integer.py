...
def safe_tensor_item(tensor_value):
    """Tensor 값을 안전하게 Python 숫자로 변환"""
...
def visualize_clusters(
    ...
    time_period: int = 0,  # items -> time_period로 변경
    ...
    for i in range(len(locations)):
        demand = safe_tensor_item(net_demand[i, time_period])  # 안전한 변환
        if demand < 0:  # 수요 (음수)
    ...
    # 노드 크기: 수요/공급 절대값에 비례
    node_sizes = [300 + 100 * abs(safe_tensor_item(net_demand[i, time_period])) for i in range(len(locations))]
    ...
    if valid_clusters:
        ...
        for cluster_idx, cluster in enumerate(valid_clusters):
            ...
            # 클러스터별 범례 추가
            cluster_demand = sum(safe_tensor_item(net_demand[node_idx, time_period]) for node_idx in cluster)
            try:
    ...
    # 제목 및 레이블
    try:
        title = f'강원도 노드 클러스터링 결과 (품목 {time_period+1}) - 유효한 클러스터만 표시'
        xlabel = 'MDS 좌표 X'
        ylabel = 'MDS 좌표 Y'
    except:
        title = f'Gangwon Province Node Clustering Result (Items {time_period+1}) - Valid Clusters Only'
...
def plot_fitness_history(fitness_history: List) -> plt.Figure:
    """
    적합도 역사를 시각화하는 함수
    ...
    # fitness_history의 각 요소를 안전하게 변환
    safe_fitness_history = [safe_tensor_item(f) if hasattr(f, 'item') else f for f in fitness_history]
    
    ax.plot(safe_fitness_history, '-o', markersize=4, alpha=0.7)
...
def analyze_clusters(
    ...
    for cluster_idx, cluster in enumerate(clusters):
        ...
        for item_idx in range(num_items):
            ...
            for node_idx in cluster:
                value = safe_tensor_item(net_demand[node_idx, item_idx])  # 안전한 변환
                if value < 0:
        ...
        if len(cluster) > 1:
            ...
            for i in range(len(cluster)):
                for j in range(i+1, len(cluster)):
                    node_i, node_j = cluster[i], cluster[j]
                    total_dist += safe_tensor_item(dist_matrix[node_i, node_j])  # 안전한 변환
                    pair_count += 1
...
def print_cluster_report(
    ...
    net_demand: torch.Tensor,
    analysis: Dict,
    split_ratios: Dict[Tuple[int, int], torch.Tensor] = None,  # 타입 수정
    min_nodes_per_cluster: int = 2,
    balance_tolerance: float = 0.1,
    ...
    Args:
        ...
        split_ratios: 노드별, 클러스터별 분할된 수요/공급 텐서
    ...
    for cluster_idx, cluster in enumerate(clusters):
        ...
        for node_idx in cluster:
            ...
            if split_ratios and (node_idx, cluster_idx) in split_ratios:
                split_tensor = split_ratios[(node_idx, cluster_idx)]
                split_demand_values = [safe_tensor_item(split_tensor[t]) for t in range(split_tensor.shape[0])]
                demand_str = ", ".join([f"{val:.2f}" for val in split_demand_values])
                
                # 원래 값 표시
                original_values = [safe_tensor_item(net_demand[node_idx, t]) for t in range(net_demand.shape[1])]
                ...
                if overlapping:
                    if use_english:
                        print(f"  - {node_name} (Split Demand/Supply: [{demand_str}], Original: [{original_str}])")
                    else:
                        try:
                            print(f"  - {node_name} (분할된 수요/공급: [{demand_str}], 원래: [{original_str}])")
                        except:
                            print(f"  - {node_name} (Split Demand/Supply: [{demand_str}], Original: [{original_str}])")
                else:
                    if use_english:
                ...
                demand_values = [safe_tensor_item(net_demand[node_idx, t]) for t in range(net_demand.shape[1])]
...
def visualize_clusters_by_cluster(
    ...
    # 클러스터별 시각화
    for i, (cluster_idx, cluster) in enumerate(all_clusters):
        ...
        for item_idx in range(net_demand.shape[1]):
            cluster_net_demand = sum(safe_tensor_item(net_demand[node_idx, item_idx]) for node_idx in cluster)
        ...
        for j in cluster:
            ...
            total_demand = sum(1 for t in range(net_demand.shape[1]) if safe_tensor_item(net_demand[j, t]) < 0)
            total_supply = sum(1 for t in range(net_demand.shape[1]) if safe_tensor_item(net_demand[j, t]) > 0)
            ...
            for t in range(net_demand.shape[1]):
                value = safe_tensor_item(net_demand[j, t])
                if value < 0:
        ...
        balance_info = []
        for t in range(net_demand.shape[1]):
            cluster_demand = sum(safe_tensor_item(net_demand[node_idx, t]) for node_idx in cluster)
