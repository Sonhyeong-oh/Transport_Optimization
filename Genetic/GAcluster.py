...
# 모델 및 유틸리티 임포트
from GAcluster_model import GeneticClusteringAlgorithm, ClusteringSolution
from GAcluster_utils import visualize_clusters, visualize_clusters_by_cluster, plot_fitness_history, analyze_clusters, print_cluster_report
...
def main(args):
    ...
    if debug_mode:
        ...
        for i, solution in enumerate(ga.population):
            print(f"솔루션 {i}: 적합도 = {solution.fitness}")
    
    # 알고리즘 실행
    try:
        print("\n유전 알고리즘 실행 중...")
        best_solution, fitness_history = ga.run(num_generations=args.generations)
        print(f"완료! 최종 적합도: {best_solution.fitness:.4f}")
        ...
        if valid_solutions:
            ...
            print(f"오류 후 복구된 최적 솔루션 적합도: {best_solution.fitness:.4f}")
        else:
    ...
    try:
        # 노드 분할 비율 계산
        split_ratios = ga.split_node_demands(best_solution)
