...
class GeneticClusteringAlgorithm:
    ...
    def split_node_demands_integer(self, solution: ClusteringSolution, net_demand: torch.Tensor = None) -> Dict[Tuple[int, int], torch.Tensor]:
        """
        중복 노드의 수요/공급을 클러스터 간에 정수로 분할합니다.
    
        Args:
            solution: 클러스터링 솔루션
            net_demand: 노드별 품목별 수요/공급 데이터 (정수). None인 경우 self.net_demand 사용
    
        Returns:
            Dict[Tuple[int, int], torch.Tensor]: (노드 인덱스, 클러스터 인덱스) -> 분할된 수요/공급 텐서 매핑
        """
        # net_demand가 제공되지 않으면 인스턴스의 net_demand 사용
        if net_demand is None:
            net_demand = self.net_demand
        
        threshold = 0.5  # 클러스터 소속 임계값
        split_demands = {}  # 분할된 수요/공급 저장
        ...
        for node_idx in range(self.num_nodes):
            ...
            for cluster_idx in range(self.num_clusters):
                if solution.membership_matrix[node_idx, cluster_idx] >= threshold:
                    belongs_to_clusters.append(cluster_idx)
            
            if len(belongs_to_clusters) == 0:
                # 어떤 클러스터에도 속하지 않는 경우, 가장 높은 멤버십을 가진 클러스터에 할당
                best_cluster = torch.argmax(solution.membership_matrix[node_idx]).item()
                belongs_to_clusters = [best_cluster]
            
            if len(belongs_to_clusters) == 1:
                # 하나의 클러스터에만 속한 경우 - 전체 수요/공급을 그 클러스터에 할당
                cluster_idx = belongs_to_clusters[0]
                split_demands[(node_idx, cluster_idx)] = net_demand[node_idx].clone()
            else:
                # 여러 클러스터에 속한 경우 - 정수 분할 수행
                node_demand = net_demand[node_idx]  # shape: (num_items,)
                
                # 멤버십 값에 비례한 분할 비율 계산
                membership_values = [solution.membership_matrix[node_idx, c].item() for c in belongs_to_clusters]
                total_membership = sum(membership_values)
                ratios = [m / total_membership for m in membership_values]
                
                # 각 품목별로 정수 분할
                for item_idx in range(self.num_items):
                    original_value = int(node_demand[item_idx].item())
                    
                    if original_value == 0:
                        # 0인 경우 모든 클러스터에 0 할당
                        for cluster_idx in belongs_to_clusters:
                            if (node_idx, cluster_idx) not in split_demands:
                                split_demands[(node_idx, cluster_idx)] = torch.zeros(self.num_items, dtype=torch.int32)
                            split_demands[(node_idx, cluster_idx)][item_idx] = 0
                        continue
                    
                    # 비율에 따라 초기 분할
                    split_values = []
                    remaining = abs(original_value)
                    
                    for i, ratio in enumerate(ratios[:-1]):  # 마지막 클러스터 제외
                        allocated = int(ratio * abs(original_value))
                        split_values.append(allocated)
                        remaining -= allocated
                    
                    # 남은 값을 마지막 클러스터에 할당
                    split_values.append(remaining)
                    
                    # 원래 값의 부호 적용
                    if original_value < 0:
                        split_values = [-v for v in split_values]
                    
                    # 분할 결과 저장
                    for i, cluster_idx in enumerate(belongs_to_clusters):
                        if (node_idx, cluster_idx) not in split_demands:
                            split_demands[(node_idx, cluster_idx)] = torch.zeros(self.num_items, dtype=torch.int32)
                        split_demands[(node_idx, cluster_idx)][item_idx] = split_values[i]
        
        return split_demands
    ...
    def evaluate_fitness(self, solution: ClusteringSolution) -> torch.Tensor:
        ...
        try:
            ...
            # 정수 분할로 노드 수요/공급 분할 계산
            split_demands = self.split_node_demands_integer(solution, self.net_demand)
            ...
            for cluster_idx, cluster in enumerate(clusters):
                if len(cluster) == 0:
                    ...
                    cluster_size_penalty += 1000.0 * (self.min_nodes_per_cluster - len(cluster))
            ...
            for cluster_idx, cluster in enumerate(clusters):
                ...
                for item_idx in range(self.num_items):
                    # 클러스터 내 모든 노드의 수요/공급 합산 (정수 분할 적용)
                    cluster_net_demand = 0

                    for node_idx in cluster:
                        # 정수 분할된 수요/공급 값 가져오기
                        if (node_idx, cluster_idx) in split_demands:
                            node_demand = split_demands[(node_idx, cluster_idx)][item_idx].item()
                            cluster_net_demand += node_demand

                    # 허용 오차 범위 (정수이므로 0으로 설정)
                    tolerance = 0
