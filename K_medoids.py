...
def constrained_k_medoids(dist_matrix, n_clusters, max_iter=10000, threshold=100, random_state=0):
    ...
    # ✅ 1. 초기 medoid 선택 (KMeans++ 스타일)
    medoids = [np.random.choice(n_samples)]  # 첫 번째 medoid 무작위
    for _ in range(1, n_clusters):
        # 기존 medoid들과의 최소 거리 계산
        min_dists = np.min(dist_matrix[:, medoids], axis=1)
        min_dists[medoids] = 0  # 이미 선택된 건 제외
        probs = min_dists / np.sum(min_dists)
        next_medoid = np.random.choice(n_samples, p=probs)
...
visualize_clusters(G, pos, labels, title="K-Medoids Result")
print_cluster_details_and_paths(fixed_net_demand.numpy(), labels, tf_dist_matrix, G, pos)

init_labels = best['labels']
best_labels, best_score = multiple_runs_sa(
    fixed_net_demand, init_labels, dist_matrix,
...
# 각 클러스터별 결과 출력 & 시각화
visualize_clusters(G, pos, best_labels, title="Metaheuristic Optimization Result")
print_cluster_details_and_paths(fixed_net_demand.numpy(), best_labels, tf_dist_matrix, G, pos)

# 총 불균형 점수 계산
imbalance_score = supply_demand_imbalance_score(fixed_net_demand.numpy(), best_labels)
